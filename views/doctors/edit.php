...
<form class="panel form-grid" method="post" action="<?= e(url('/doctors/' . ($data['id'] ?? $_GET['id'] ?? ''))) ?>" novalidate>
    <?= csrf_field() ?>

    <div>
        <label for="license_number">Numero de licencia</label>
        <input id="license_number" name="license_number" maxlength="30" value="<?= e($data['license_number'] ?? '') ?>" required>
    ...
    <div>
        ...
        <select id="specialty" name="specialty" required>
            ...
            <option value="Cardiología" <?= ($data['specialty'] ?? '') === 'Cardiología' ? 'selected' : '' ?>>Cardiología</option>
            <option value="Neurología" <?= ($data['specialty'] ?? '') === 'Neurología' ? 'selected' : '' ?>>Neurología</option>
            <option value="Pediatría" <?= ($data['specialty'] ?? '') === 'Pediatría' ? 'selected' : '' ?>>Pediatría</option>
        </select>
